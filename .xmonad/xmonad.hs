...
main = do
    ...
    xmproc0 <- spawnPipe "xmobar -x 0 /home/aleksey/.xmonad/xmobar.hs"
...
------------------------------------------------------------------------
myStartupHook = do
          spawnOnce "feh --bg-scale /home/aleksey/.config/i3/bg-icescape.jpg" -- Sets wallpaper
          spawnOnce "compton -b" 
          spawnOnce "date > /home/aleksey/date.txt"
          spawnOnce "xset r rate 200 40"
          spawnOnce "xmodmap -e 'keycode 135 = Super_L'"
          setWMName "LG3D"
...
myKeys =
    ...
    -- Layouts
        , ("M-t", sendMessage NextLayout)                              -- Switch to next layout
    ...
    -- Workspaces
        --, ("M-l", shiftToNext)                               		-- Go to next workspace
        --, ("M-h", shiftToNext)                           			-- Go to previous workspace
        , ("M-<Left>", moveTo Next nonNSP)                               		-- Go to next workspace
        , ("M-<Right>", moveTo Prev nonNSP)                           			-- Go to previous workspace
    ...
    --- Scripts
        , ("M-<Return>", spawn "layout=$(xkb-switch -p); xkb-switch -s us; rofi -font 'hack 20' -them solarized -show run; xkb-switch -s ${layout}")
        , ("M-<Space>", spawn "xkb-switch -n")
    ...
    -- Multimedia Keys
        ...
        , ("M-<F11>", spawn "pactl set-sink-volume @DEFAULT_SINK@ -5% && $refresh_i3status") -- volume down
        , ("M-<F12>", spawn "pactl set-sink-volume @DEFAULT_SINK@ +5% && $refresh_i3status") -- volume up

        , ("M-<F7>", spawn "playerctl previous")
        , ("M-<F8>", spawn "playerctl play-pause")
        , ("M-<F9>", spawn "playerctl next")
...
myWorkspaces = clickable . (map xmobarEscape) 
               $ ["1:main", "2:web", "3:dev", "4:chat", "5:code", "6:media", "7:vbox", "8:gfx", "9:git"]
  where                                                                      
...
myManageHook = composeAll
     [
      ...
      , className =? "Chromium"    --> doShift "<action=xdotool key super+2>2:web</action>"
      , className =? "Gimp"        --> doShift "<action=xdotool key super+8>8:code</action>"
...
myLayoutHook = avoidStruts $ mouseResize $ windowArrange $ 
             ...
             where 
                 myDefaultLayout = tall ||| noBorders monocle ||| oneBig ||| threeRow
